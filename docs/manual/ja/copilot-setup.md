...
この設定により、Copilot が `.github/copilot-instructions.md` をチャットセッションごとに自動読み込みする。小さなモデルの場合、この設定がないと、Copilot は gospelo-kata のワークフロールールに従わないことがある。

[`skill/copilot/copilot-instructions.md`](https://github.com/gospelo-dev/kata/blob/main/skill/copilot/copilot-instructions.md) の内容をプロジェクトの `.github/copilot-instructions.md` に追記すること。
...
```
「社内ポータルリニューアル」のセキュリティチェックリストを作成してください。
出力先: docs/output/
テンプレート: checklist
