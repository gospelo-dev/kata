...
| テンプレート | 説明 | 主な用途 |
|-------------|------|---------|
| `storyboard` | 登場人物・カット・セリフ・各カット画像を扱う絵コンテ | オンボーディング、チュートリアル、短編動画、セリフモックアップ |

`storyboard` テンプレートは、キャラクターアバターとカットの
プレースホルダー画像を `.katar` に同梱しています。Data からは
`./images/storyboard/...` の形でテンプレート名前空間つきで画像を参照する
ので、画像を持つテンプレートを複数並列にインストールしても衝突しません。
2 種類のセリフモードが使えます:

- `dialogue: string[]` — 1 カット 1 話者(`cut.speaker` 指定)
- `lines: [{speaker, text}, ...]` — 1 カット内で複数話者が交互に発話。
  各セリフが話者のアバター付きの吹き出しとしてレンダーされる

実例は `docs/examples/ja/storyboard/` を参照。
