...
## <span data-kata="p-categories-0-id">injection</span>. <span data-kata="p-categories-0-name">インジェクション系</span>
...
- <span data-kata="p-categories-0-items-0-requirements">正常系: 通常パラメータで期待通りのレスポンスが返る</span>
...
<tr><td>target</td><td><span data-kata="p-categories-0-items-1-target">フロントエンド + API</span></td></tr>
...
- <span data-kata="p-categories-0-items-1-requirements">Reflected XSS: &lt;script&gt;alert(1)&lt;/script&gt;がHTMLにそのまま反映されないこと</span>
...
- <span data-kata="p-categories-0-items-2-requirements">検出: ; /bin/sleep 20 挿入でレスポンス遅延が発生しないこと (Time-based)</span>
...
- <span data-kata="p-categories-0-items-3-requirements">検出: ../../etc/passwd でシステムファイルの内容が返らないこと</span>

</td>
</tr>
</table>

## <span data-kata="p-categories-1-id">auth</span>. <span data-kata="p-categories-1-name">認証・認可</span>
...
- <span data-kata="p-categories-1-items-0-requirements">レート制限: 同一アカウントへの連続失敗（5回以上）でアカウントロックまたは遅延が発生すること</span>
...
- <span data-kata="p-categories-1-items-1-requirements">水平権限昇格: ユーザーAのトークンでユーザーBのリソース（user_id差し替え）にアクセスし403が返ること</span>
...
- <span data-kata="p-categories-1-items-2-requirements">トークン検証: CSRFトークンなしでPOST/PUT/DELETEリクエストが403で拒否されること</span>

</td>
</tr>
</table>

## <span data-kata="p-categories-2-id">config</span>. <span data-kata="p-categories-2-name">設定・ヘッダー</span>
...
- <span data-kata="p-categories-2-items-0-requirements">HSTS: Strict-Transport-Security ヘッダーが存在し max-age &gt;= 31536000 であること</span>
...
- <span data-kata="p-categories-2-items-1-requirements">スタックトレース: 不正入力で500エラー発生時にスタックトレースがレスポンスに含まれないこと</span>
...
- <span data-kata="p-categories-2-items-2-requirements">npm audit: high/critical の脆弱性が0件であること</span>
